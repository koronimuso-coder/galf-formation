      ...
      <div className="container-galf relative z-10">
        ...
        <div className="grid md:grid-cols-3 gap-8">
          {instructors.map((ins, idx) => (
            <FadeIn key={idx} delay={idx * 0.1} className="stitch-card p-6 md:p-8 rounded-3xl border border-white/5 relative overflow-hidden flex flex-col justify-between h-full stitch-hud-corner group">
              <div className="space-y-4">
                {/* Header info */}
                <div className="flex justify-between items-start gap-4">
                  <div className="space-y-1">
                    <span className="text-[9px] font-mono text-galf-yellow uppercase tracking-widest block">{ins.role}</span>
                    <h3 className="text-base font-black text-white uppercase tracking-wide leading-tight group-hover:text-galf-yellow transition-colors">
                      {ins.name}
                    </h3>
                    <span className="text-[10px] text-zinc-500 font-sans block">{ins.exp}</span>
                  </div>
                  ...
                  <div className="bg-galf-yellow/10 border border-galf-yellow/20 rounded-lg px-2.5 py-1 text-center shrink-0">
                    ...
                    <span className="text-[7px] text-zinc-500 uppercase tracking-widest font-mono">Score HSE</span>
                  </div>
                </div>
                ...
                <div className="flex flex-wrap gap-2 pt-2">
                  {ins.badges.map((b, bIdx) => (
                    <span 
                      key={bIdx}
                      className="px-2.5 py-1 rounded bg-zinc-900 border border-white/5 text-[8px] font-black tracking-widest text-white/60 uppercase"
                    >
                      {b}
                    </span>
                  ))}
              ...
              <div className="mt-8 pt-4 border-t border-white/5 flex items-center justify-between text-[10px] font-mono text-zinc-500 uppercase tracking-widest">
                ...
                <span className="text-emerald-500 font-bold flex items-center gap-1"><ShieldCheck className="w-3.5 h-3.5" /> Actif</span>
              </div>
            </FadeIn>
          ))}
        </div>
