    ...
    <div className="stitch-card p-6 md:p-8 rounded-3xl border border-white/5 relative overflow-hidden bg-zinc-950/40 stitch-hud-corner shadow-2xl">
      ...
      <div className="relative z-10 space-y-6">
        <div>
          ...
          <h3 className="text-xl font-black text-white uppercase tracking-tight">Répartition des Heures de Vol</h3>
        ...
        {/* Circular gauge */}
        <div className="flex flex-col sm:flex-row items-center gap-8 bg-black/30 p-6 rounded-2xl border border-white/5">
          <div className="relative w-32 h-32 flex items-center justify-center shrink-0">
            ...
            <div className="absolute text-center">
              <span className="text-2xl font-black text-white block leading-none">{details.cabin}h</span>
              <span className="text-[9px] font-mono text-zinc-500 uppercase tracking-wider block mt-1">Cabine Réelle</span>
            </div>
          </div>

          <div className="space-y-2 font-sans text-xs">
            <h4 className="text-sm font-black text-white uppercase">Engagement Qualité GALF</h4>
            ...
            <div className="flex items-center gap-2 text-[10px] font-mono text-galf-yellow uppercase tracking-widest pt-1">
        ...
        <div className="grid md:grid-cols-2 gap-4 font-sans text-xs pt-2">
          {items.map((item, idx) => (
            <div key={idx} className="p-4 rounded-xl bg-black/20 border border-white/5 hover:border-white/10 transition-all flex gap-3 items-start">
              <div className={`w-8 h-8 rounded-lg bg-zinc-900 border border-white/5 flex items-center justify-center shrink-0 ${item.textColor}`}>
                <item.icon className="w-4 h-4" />
              </div>
              <div className="space-y-1">
                <div className="flex justify-between items-baseline gap-2">
                  <span className="font-bold text-white uppercase text-[11px] leading-tight">{item.label}</span>
                  <span className="font-mono font-black text-white shrink-0">{item.hours}h</span>
                </div>
