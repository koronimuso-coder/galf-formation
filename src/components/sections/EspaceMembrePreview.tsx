...
export function EspaceMembrePreview() {
  ...
  return (
    <section className="py-24 border-t border-white/5 relative overflow-hidden bg-zinc-950/40">
        ...
        <div className="grid lg:grid-cols-12 gap-12 items-center">
          ...
          <div className="lg:col-span-5 space-y-6">
            ...
            <h2 className="text-3xl md:text-5xl font-black text-white uppercase tracking-tight leading-tight">
              Préparez-vous <br />
              <span className="text-galf-yellow">en ligne</span>
            </h2>
            ...
            <div className="space-y-4 pt-2">
              <div className="flex items-start gap-3">
                <span className="w-5 h-5 rounded-full bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 flex items-center justify-center text-xs shrink-0 font-bold">✓</span>
                <div>
                  <h4 className="text-xs font-black text-white uppercase">Accès Instantané après Inscription</h4>
                  <p className="text-[10px] text-zinc-400 font-sans">Compte étudiant créé automatiquement.</p>
                </div>
              </div>
              <div className="flex items-start gap-3">
                <span className="w-5 h-5 rounded-full bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 flex items-center justify-center text-xs shrink-0 font-bold">✓</span>
                <div>
                  <h4 className="text-xs font-black text-white uppercase">Suivi de Progression par Instructeur</h4>
                  <p className="text-[10px] text-zinc-400 font-sans">Votre instructeur valide vos quiz théoriques à distance.</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="lg:col-span-7">
            <FadeIn className="stitch-card p-6 md:p-8 rounded-[2.5rem] border border-white/5 relative overflow-hidden bg-zinc-950/70 shadow-2xl stitch-hud-corner">
              {/* Telemetry Header */}
              <div className="text-[9px] font-mono text-galf-yellow/60 font-black uppercase tracking-[0.2em] border-b border-white/5 pb-3 mb-6 flex justify-between items-center">
                <span>PLATEFORME D'APPRENTISSAGE // DEMO-MOCK</span>
                <span className="flex items-center gap-1"><span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-ping" /> SYS: OK</span>
              </div>

              {/* Grid of E-Learning features */}
              <div className="grid sm:grid-cols-2 gap-4">
                {modules.map((m, idx) => (
                  <div key={idx} className="p-5 rounded-2xl bg-zinc-900/60 border border-white/5 space-y-3 hover:border-galf-yellow/20 transition-all font-sans group">
                    <div className="w-10 h-10 rounded-xl bg-galf-yellow/10 border border-galf-yellow/20 flex items-center justify-center group-hover:scale-105 transition-transform shrink-0">
                      <m.icon className="w-5 h-5 text-galf-yellow" />
                    </div>
              ...
              {/* Mock platform CTA */}
              <div className="mt-6 pt-4 border-t border-white/5 flex flex-col sm:flex-row justify-between items-center gap-4 text-xs">
                <span className="text-[10px] font-mono text-zinc-500 uppercase tracking-widest">
                  Lancement officiel : Fin 2026
                </span>
                <button 
                  disabled
                  className="bg-galf-yellow/10 text-galf-yellow border border-galf-yellow/30 px-6 py-2.5 rounded-xl font-black text-[10px] uppercase tracking-widest opacity-60 cursor-not-allowed"
                >
